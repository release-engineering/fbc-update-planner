...
| Requested `-p` package not found | PLCC filtering | Error (exit 3) in FBC generation mode; with `--allow-missing` warning only. The `report` command classifies missing packages as `PLCC missing` (no error). |
