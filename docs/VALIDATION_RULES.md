...
| Requested `-p` package not found | PLCC filtering | Error (exit 3) in FBC generation mode; with `--allow-missing` warning only. In `--report` mode, missing packages are classified as `PLCC missing` (no error). |
