...
After PLCC data is fetched, each product passes through three validation stages:

1. **PLCC-level validation** (`pkg/plcc/validation.go`): validators that check raw PLCC data quality (tier, release cadence, phase completeness, dates). By default these produce **warnings**; with `--strict` they filter out failing packages.
2. **Pre-pipeline checks** (`TranslateAndValidate` in `pkg/fbc/fbc.go`): rules that **operate across all raw entries** (for invalid and duplicate detection).
3. **Filter pipeline** (`pkg/fbc/filter.go`): an ordered sequence of `Filter` callbacks that can mutate, validate, or reject **a single package**.

A package is emitted as an FBC blob only if it passes stages 2 and 3. Stage 1 produces warnings (or filters with `--strict`).
...
---
### Pre-Pipeline: PLCC-Level Validation
